...
function App() {
  ...
  const loadActiveHunt = async () => {
    ...
    if (hunt) {
      setActiveHunt(hunt);
      if (hunt.hunt_started && user && !isAdmin) {
        await loadMyProgress(hunt.id);
      }
      if (isAdmin && hunt.hunt_started) {
  ...
  const endHunt = async () => {
    ...
    if (confirm('End hunt for everyone?')) {
      ...
      await supabaseClient.from('profiles').update({ display_name: null, pet: null }).neq('id', '00000000-0000-0000-0000-000000000000');
      setActiveHunt(null);
      setMyProgress([]);
      setAllProgress([]);
      setPlayers([]);
  ...
  const playerInLobby = players.find(p => p.id === user?.id);
