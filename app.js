...
function App() {
  ...
  const loadActiveHunt = async () => {
    ...
    if (!error && data) {
      ...
      const wasNotStarted = !activeHunt?.hunt_started;
      setActiveHunt(data);
      
      if (data.hunt_started && user && !isAdmin) {
        await loadMyProgress(data.id);
        // Start timer when hunt first starts for this player
        if (wasNotStarted && !clueStartTime) {
          console.log('Starting timer for first clue');
          setClueStartTime(new Date());
        }
      }
  ...
  const createHunt = async (size) => {
    ...
    // Get all previous hunts to track used clues
    const { data: previousHunts } = await supabaseClient.from('hunts').select('clues');
    const usedClueIds = new Set();
    
    if (previousHunts) {
      previousHunts.forEach(hunt => {
        if (hunt.clues) {
          hunt.clues.forEach(clue => usedClueIds.add(clue.id));
        }
      });
    }
    
    // Filter out used clues, or reset if all have been used
    let availableClues = CLUES.filter(clue => !usedClueIds.has(clue.id));
    if (availableClues.length < size) {
      // All clues have been used, reset and use all
      availableClues = [...CLUES];
    }
    
    // Deactivate old hunts
    await supabaseClient.from('hunts').update({ is_active: false }).eq('is_active', true);
    
    // Select and randomize clues
    const shuffled = [...availableClues].sort(() => Math.random() - 0.5);
    ...
    const { data: hunt, error } = await supabaseClient.from('hunts').insert({ 
      ...
      is_active: true, 
      hunt_started: false,
      win_message: winMessage || 'Congratulations! 🎉',
      created_by: user.id 
    }).select().single();
  ...
  const submitPassword = async () => {
    ...
    if (passwordInput.toUpperCase() === currentClue.password.toUpperCase()) {
      const now = new Date();
      const timeTaken = clueStartTime ? Math.floor((now - clueStartTime) / 1000) : 0;
      
      console.log('Clue completed:', {
        clueIndex: currentClueIndex,
        startTime: clueStartTime,
        endTime: now,
        timeTaken,
        timeTakenMinutes: Math.floor(timeTaken / 60)
      });
      ...
      if (!error) {
        ...
        console.log('Starting timer for next clue:', nextStartTime);
        setClueStartTime(nextStartTime); // Start timer for next clue
      } else {
  ...
  const getPlayerStats = () => {
    return players.map(p => {
      ...
      const totalTime = progress.reduce((sum, pr) => sum + (pr.time_taken || 0), 0);
      const avgTime = progress.length > 0 ? Math.round(totalTime / progress.length) : 999999; // High default for sorting
      const lastClueTime = progress.length > 0 ? progress[progress.length - 1]?.time_taken || 0 : 0;
      ...
      // First: Sort by completion (completed players first)
      if (a.isComplete !== b.isComplete) return b.isComplete - a.isComplete;
      // Second: Sort by clues found (more clues = higher rank)
      if (a.cluesFound !== b.cluesFound) return b.cluesFound - a.cluesFound;
      // Third: Sort by average time (faster = higher rank)
      ...
      <div style={{maxWidth:'1000px',margin:'0 auto'}}>
        {isAdmin&&!activeHunt?(
          <div style={{background:'rgba(255,255,255,0.98)',borderRadius:'25px',padding:'50px',boxShadow:'0 15px 50px rgba(0,0,0,0.3)',textAlign:'center'}}>
            ...
            <p style={{fontSize:'18px',color:'#666',marginBottom:'30px'}}>Set up a scavenger hunt!</p>
            
            <div style={{background:'linear-gradient(135deg,#fef3c7 0%,#fde68a 100%)',borderRadius:'20px',padding:'25px',marginBottom:'40px',border:'3px solid #f59e0b',textAlign:'left'}}>
              <h3 style={{fontSize:'18px',fontWeight:'bold',marginBottom:'15px',color:'#92400e'}}>💡 Pro Tip: Hiding Passwords</h3>
              <p style={{fontSize:'15px',lineHeight:'1.6',color:'#78350f',margin:0}}>
                <strong>BEST:</strong> Hide the password <strong>directly on, in, or under</strong> the object/thing where <strong>it's NOT VISIBLE</strong>. Aim to do this whenever possible!<br/>
                <strong>IF NOT POSSIBLE:</strong> Place the password <strong>near</strong> the object with a bit sticking out so it's visible but not obvious.
              </p>
            </div>
            
            <div style={{background:'linear-gradient(135deg,#f8f9fa 0%,#e9ecef 100%)',borderRadius:'20px',padding:'40px',marginBottom:'25px',border:'3px solid #667eea'}}>
            ...
            <div style={{background:'linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%)',borderRadius:'20px',padding:'30px',marginBottom:'40px',border:'3px solid #10b981'}}>
              <label style={{display:'block',fontSize:'18px',fontWeight:'600',color:'#059669',marginBottom:'15px'}}>Win Message (optional)</label>
              <input type="text" placeholder="Congratulations! 🎉" value={winMessage} onChange={e=>setWinMessage(e.target.value)} maxLength={100} style={{width:'100%',padding:'16px',fontSize:'16px',border:'2px solid #10b981',borderRadius:'12px',outline:'none'}}/>
              <p style={{fontSize:'13px',color:'#666',marginTop:'10px'}}>Players will see this when they finish</p>
            </div>
          ...
          <div style={{background:'rgba(255,255,255,0.98)',borderRadius:'25px',padding:'40px',boxShadow:'0 15px 50px rgba(0,0,0,0.3)'}}>
            <div style={{textAlign:'center',marginBottom:'30px'}}>
              ...
              <button onClick={()=>setShowAdminPasswords(!showAdminPasswords)} style={{padding:'10px 20px',background:'#667eea',color:'white',border:'none',borderRadius:'10px',fontSize:'14px',fontWeight:'600',cursor:'pointer',marginTop:'10px'}}>
                {showAdminPasswords?'Hide':'Show'} Passwords & Locations
              </button>
            </div>
            
            {showAdminPasswords&&<div style={{background:'linear-gradient(135deg,#fef3c7 0%,#fde68a 100%)',borderRadius:'20px',padding:'25px',marginBottom:'25px',border:'3px solid #f59e0b'}}>
              <h3 style={{fontSize:'18px',fontWeight:'bold',marginBottom:'15px',color:'#92400e'}}>🔑 Passwords & Locations</h3>
              <div style={{display:'grid',gap:'8px',maxHeight:'300px',overflowY:'auto'}}>
                {activeHunt.clues.map((clue,i)=>(
                  <div key={i} style={{background:'white',padding:'12px 16px',borderRadius:'8px',display:'flex',justifyContent:'space-between',alignItems:'center',gap:'12px'}}>
                    <div style={{display:'flex',alignItems:'center',gap:'10px',flex:1}}>
                      <div style={{width:'24px',height:'24px',background:'#667eea',borderRadius:'6px',display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontWeight:'bold',fontSize:'12px'}}>{i+1}</div>
                      <div style={{fontFamily:'Courier New',color:'#667eea',fontWeight:'bold',fontSize:'14px'}}>{clue.password}</div>
                    </div>
                    <div style={{fontSize:'13px',color:'#10b981',fontWeight:'600',textAlign:'right'}}>📍 {CLUES.find(c=>c.id===clue.id)?.answer}</div>
                  </div>
                ))}
              </div>
            </div>}
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:'15px',flexWrap:'wrap'}}>
                      <div style={{display:'flex',alignItems:'center',gap:'12px',flex:1}}>
                        ...
                        <div>
                          ...
                          <div style={{fontSize:'13px',color:'#666'}}>Currently on clue {p.cluesFound+1}/{activeHunt.total_clues}</div>
                          {p.cluesFound>0&&<div style={{fontSize:'12px',color:'#059669',marginTop:'2px'}}>
                            ⏱️ Avg: {formatTime(p.avgTime)} | Last: {formatTime(p.lastClueTime)} | Total: {formatTime(p.totalTime)}
                          </div>}
                        </div>
                      </div>
        ...
        ):activeHunt && !activeHunt.hunt_started && !isAdmin && !playerInLobby?(
          <div style={{background:'rgba(255,255,255,0.98)',borderRadius:'25px',padding:'60px 40px',boxShadow:'0 15px 50px rgba(0,0,0,0.3)',textAlign:'center'}}>
          ...
          <div style={{background:'rgba(255,255,255,0.98)',borderRadius:'25px',padding:'60px 40px',boxShadow:'0 15px 50px rgba(0,0,0,0.3)',textAlign:'center',position:'relative',overflow:'hidden'}}>
            ...
            <p style={{fontSize:'24px',color:'#333',marginBottom:'20px',fontWeight:'600'}}>{activeHunt.win_message || 'Congratulations! 🎉'}</p>
            <p style={{fontSize:'18px',color:'#666',marginBottom:'30px'}}>You found all {activeHunt.total_clues} clues! Amazing work!</p>
