...
function App() {
  ...
  const startHunt = async () => {
    ...
    const { data, error } = await supabaseClient.from('hunts').update({ hunt_started: true }).eq('id', activeHunt.id).select().maybeSingle();
    if (!error && data) {
      setActiveHunt(data);
      console.log('Hunt started!', data);
    } else {
      console.error('Start hunt error:', error);
      alert('Failed to start hunt: ' + (error?.message || 'Unknown error'));
    }
  };
