...
function App() {
  ...
  const loadActiveHunt = async () => {
    ...
    if (!error && data) {
      ...
      // Start timer when hunt first starts
      if (data.hunt_started && wasNotStarted && !isAdmin && !clueStartTime) {
  ...
  const submitPassword = async () => {
    ...
    if (passwordInput.toUpperCase() === currentClue.password.toUpperCase()) {
      ...
      if (!error) {
        ...
        setClueStartTime(new Date()); // Start timer for next clue
      }
    } else {
  ...
  const getPlayerStats = () => {
    return players.map(p => {
      ...
      const avgTime = progress.length > 0 ? Math.round(totalTime / progress.length) : 0;
