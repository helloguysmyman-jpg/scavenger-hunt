  ...
  useEffect(() => {
    ...
    if (activeHunt && !activeHunt.hunt_started && !isAdmin && playerInLobby) {
      ...
      const gameInterval = setInterval(() => {
        ...
        setFallingItems(prev => [...prev, newItem].slice(-8)); // Max 8 items
      }, 1500);
      ...
      <div style={{maxWidth:'1000px',margin:'0 auto'}}>
        {isAdmin&&!activeHunt?(
          ...
          <div style={{background:'rgba(255,255,255,0.98)',borderRadius:'25px',padding:'40px',boxShadow:'0 15px 50px rgba(0,0,0,0.3)',textAlign:'center'}}>
            <h2 style={{fontSize:'28px',fontWeight:'bold',marginBottom:'10px',color:'#333'}}>🎮 Lobby</h2>
            ...
            {gameActive&&<div style={{position:'relative',height:'350px',background:'linear-gradient(135deg,#e0f2fe 0%,#bae6fd 100%)',borderRadius:'20px',marginBottom:'25px',overflow:'hidden',border:'3px solid #667eea'}}>
              <div style={{position:'absolute',top:'15px',right:'15px',background:'white',padding:'12px 24px',borderRadius:'12px',fontSize:'20px',fontWeight:'bold',color:'#667eea',boxShadow:'0 4px 12px rgba(0,0,0,0.1)',zIndex:10}}>🏆 {gameScore}</div>
              <div style={{position:'absolute',top:'15px',left:'15px',color:'white',fontSize:'16px',fontWeight:'600',background:'rgba(0,0,0,0.3)',padding:'8px 16px',borderRadius:'8px'}}>Tap to catch!</div>
              {fallingItems.map(item=><div key={item.id} onClick={()=>catchItem(item.id)} style={{position:'absolute',left:`${item.x}%`,top:`${item.y}%`,fontSize:'48px',cursor:'pointer',transition:'top 0.05s linear',filter:'drop-shadow(0 2px 4px rgba(0,0,0,0.2))'}}>{item.emoji}</div>)}
            </div>}
