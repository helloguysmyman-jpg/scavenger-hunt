...
function App() {
  ...
  useEffect(() => {
    if (user && activeHunt && activeHunt.hunt_started) {
      const interval = setInterval(() => loadMyProgress(activeHunt.id), 3000);
      return () => clearInterval(interval);
    }
  }, [user, activeHunt]);
      ...
      {showAdminPrompt&&(
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,0.85)',display:'flex',justifyContent:'center',alignItems:'center',zIndex:1000,padding:'20px'}}>
          <div style={{background:'white',borderRadius:'30px',padding:'50px',maxWidth:'450px',width:'100%',boxShadow:'0 30px 80px rgba(0,0,0,0.5)'}} className="fade-in">
            ...
            <input type="password" value={adminPasswordInput} onChange={e=>setAdminPasswordInput(e.target.value)} onKeyPress={e=>e.key==='Enter'&&handleAdminLogin()} placeholder="admin password" autoFocus style={{width:'100%',padding:'20px',fontSize:'20px',border:'3px solid #667eea',borderRadius:'15px',marginBottom:'25px',outline:'none',fontFamily:'inherit',textAlign:'center'}}/>
            <div style={{display:'flex',gap:'12px'}}>
              <button onClick={()=>{setShowAdminPrompt(false);setAdminPasswordInput('');}} style={{flex:1,padding:'18px',background:'#e5e7eb',color:'#333',border:'none',borderRadius:'15px',fontSize:'16px',fontWeight:'600',cursor:'pointer',fontFamily:'inherit'}}>Cancel</button>
              <button onClick={handleAdminLogin} style={{flex:1,padding:'18px',background:'linear-gradient(135deg,#667eea 0%,#764ba2 100%)',color:'white',border:'none',borderRadius:'15px',fontSize:'16px',fontWeight:'600',cursor:'pointer',fontFamily:'inherit',boxShadow:'0 8px 20px rgba(102,126,234,0.4)'}}>Enter</button>
            </div>
          </div>
        </div>
      )}
        ...
        {isAdmin&&!activeHunt?(
          ...
          <div style={{background:'rgba(255,255,255,0.98)',borderRadius:'25px',padding:'40px',boxShadow:'0 15px 50px rgba(0,0,0,0.3)'}} className="fade-in">
            <div style={{textAlign:'center',marginBottom:'30px'}}>
              ...
              <p style={{fontSize:'16px',color:'#666'}}>Print & hide clues, then start the hunt!</p>
            </div>
            
            <div style={{background:'linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%)',borderRadius:'20px',padding:'30px',marginBottom:'30px',border:'3px solid #10b981'}}>
              <h3 style={{fontSize:'20px',fontWeight:'bold',marginBottom:'20px',color:'#059669'}}>👥 Players Joined ({players.length})</h3>
              <div style={{display:'flex',flexWrap:'wrap',gap:'10px'}}>
                {players.map(p=><div key={p.id} style={{background:'white',padding:'10px 20px',borderRadius:'10px',fontSize:'16px',fontWeight:'600',color:'#333',boxShadow:'0 2px 8px rgba(0,0,0,0.1)'}}>{p.display_name || 'Player'}</div>)}
              </div>
            </div>

            <div style={{background:'linear-gradient(135deg,#f8f9fa 0%,#e9ecef 100%)',borderRadius:'20px',padding:'30px',marginBottom:'30px',border:'3px solid #667eea'}}>
              <h3 style={{fontSize:'22px',fontWeight:'bold',marginBottom:'25px',color:'#333'}}>🔑 Passwords & Locations</h3>
              <div style={{display:'grid',gap:'15px',maxHeight:'300px',overflowY:'auto'}}>
                  ...
                  <div key={i} style={{background:'white',borderRadius:'15px',padding:'20px',boxShadow:'0 4px 15px rgba(0,0,0,0.08)',border:'2px solid #e5e7eb'}}>
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:'20px',flexWrap:'wrap'}}>
                      <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
                        ...
                        <div style={{fontSize:'16px',fontWeight:'600',color:'#333'}}>Password: {clue.password}</div>
                      </div>
                      {showAnswers&&<div style={{fontSize:'14px',color:'#10b981',fontWeight:'600'}}>📍 {CLUES.find(c=>c.id===clue.id)?.answer}</div>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{display:'flex',gap:'10px',justifyContent:'center',flexWrap:'wrap'}}>
              <button onClick={()=>setShowAnswers(!showAnswers)} style={{padding:'16px 30px',background:showAnswers?'#f59e0b':'#667eea',color:'white',border:'none',borderRadius:'12px',fontSize:'16px',fontWeight:'600',cursor:'pointer'}}>{showAnswers?'Hide':'Show'} Locations</button>
              ...
              <button onClick={startHunt} style={{padding:'16px 30px',background:'linear-gradient(135deg,#10b981 0%,#059669 100%)',color:'white',border:'none',borderRadius:'12px',fontSize:'18px',fontWeight:'bold',cursor:'pointer',boxShadow:'0 8px 20px rgba(16,185,129,0.4)'}} className="pulse">▶️ START HUNT!</button>
          ...
          <div style={{background:'rgba(255,255,255,0.98)',borderRadius:'25px',padding:'40px',boxShadow:'0 15px 50px rgba(0,0,0,0.3)',textAlign:'center'}} className="fade-in">
            <h2 style={{fontSize:'32px',fontWeight:'bold',marginBottom:'20px',color:'#333'}}>🎮 Hunt In Progress!</h2>
            <p style={{fontSize:'18px',color:'#666',marginBottom:'30px'}}>Players are hunting...</p>
            <button onClick={endHunt} style={{padding:'16px 30px',background:'#ef4444',color:'white',border:'none',borderRadius:'12px',fontSize:'16px',fontWeight:'600',cursor:'pointer'}}>End Hunt</button>
          </div>
        ):!activeHunt?(
          <div style={{background:'rgba(255,255,255,0.98)',borderRadius:'25px',padding:'60px 40px',boxShadow:'0 15px 50px rgba(0,0,0,0.3)',textAlign:'center'}} className="fade-in">
            ...
            <p style={{fontSize:'18px',color:'#666',marginBottom:'30px'}}>The admin is setting up the hunt...</p>
            <div style={{background:'linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%)',borderRadius:'15px',padding:'20px',border:'3px solid #10b981'}}>
              ...
              <div style={{display:'flex',flexWrap:'wrap',gap:'8px',justifyContent:'center'}}>
                {players.map(p=><div key={p.id} style={{background:'white',padding:'8px 16px',borderRadius:'8px',fontSize:'14px',fontWeight:'600',color:'#333'}}>{p.display_name || 'Player'}</div>)}
              </div>
            </div>
          </div>
          ...
          <div style={{background:'rgba(255,255,255,0.98)',borderRadius:'25px',padding:'60px 40px',boxShadow:'0 15px 50px rgba(0,0,0,0.3)',textAlign:'center'}} className="fade-in">
            ...
            <p style={{fontSize:'18px',color:'#666',marginBottom:'30px'}}>Admin is hiding the clues...</p>
            <div style={{background:'linear-gradient(135deg,#f0fdf4 0%,#dcfce7 100%)',borderRadius:'15px',padding:'20px',border:'3px solid #10b981'}}>
